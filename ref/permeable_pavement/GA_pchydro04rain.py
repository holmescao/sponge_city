...
def evalVars(Vars):  # 定义目标函数（含约束）
    ...
    for i in range(2, int(181 / Tstep)):
        ...
        if theta1[i - 1] > theta_FC1:
            ...
            if f1[i] == 0:
                delta_theta1 = math.sqrt(
                    Ks1) / Res1 * (theta1[i - 1] - theta1[0]) * Tstep
                # delta_theta1 = math.sqrt(Ks1) / Res1 * (phi1 - theta1[0]) * Tstep
        ...
        if theta2[i - 1] > theta_FC2:
            ...
            if f2[i] == L1 * (theta1[i - 1] - theta_FC1) / Tstep and f2[i] < f3[i - 1]:
                delta_theta2 = math.sqrt(
                    Ks2) / Res2 * (theta2[i - 1] - theta2[0]) * Tstep
        ...
        if d3[i - 1] <= L4:
            ...
            if theta2[i - 1] < phi2 and theta2[i - 1] >= theta_FC2:
                ...
                if theta1[i - 1] < phi1 and theta1[i - 1] >= theta_FC1:
                    h.append(
                        L3 - L4 + L2 + (theta1[i - 1] - theta_FC1) / (phi1 - theta_FC1) * L1)
                else:
...
problem = ea.Problem(name='soea quick start demo',
                     ...
                     lb=[0.3,   0.10,    0,      10,     0,      0.03,   0,  0.4,
                         0.1,     0,       0,      0,      0.03,   0, 0.2, 0, 0],  # 决策变量下界
                     ub=[0.55,  0.30,    0.03,   300,    200,    0.1,    1,  0.7,
                         0.3,     0.03,    300,    200,    0.1,    1, 0.4, 1, 2],  # 决策变量上界
