...
InputData1 = np.loadtxt('obsnumberoutflow20210525_0628.txt')
Qnumberobs = InputData1[:, 0]  # 实测出流流量的对应分钟序号
q3obs = InputData1[:, 1]  # 实测出流流量(仅包括不为0的数据），mm/hr
# 输入计算蒸散发所需的气象数据
InputData2 = np.loadtxt('meteor1min20210525_0628.txt')
...
y = 0.665*(10** (-3))*InputData2[:, 5]  # 将大气压kpa转换为Psychrometric constant，kPa/℃
...
xita2_0 = 0.57           # soil layer initial moisture content (volume of water / total volume of soil)
d3_0 = 300    # initial depth of water in the storage layer (mm)
F_0 = 0   # initial accumulative infiltraion of surface water into the soil layer (mm),500(20200521)0500
...
phi1 = 0.95 # void fraction of any surface volume (i.e., the fraction of freeboard above the surface not filled with vegetation)
...
xita2 = xitaWP*np.ones(LoopCount)   # soil layer moisture content (volume of water / total volume of soil)
d3 = np.zeros(LoopCount)              # depth of water in the storage layer (mm)
h3 = np.zeros(LoopCount)              # suction head of the storage drain (mm)     
ia = np.zeros(LoopCount) # the effective rainfall, usually = i + q0 +d1 (mm/hr)
...
e2 = np.zeros(LoopCount) # soil layer ET rate (mm/hr),PM公式,考虑水分胁迫系数的SW模型计算蒸散发量，mm/h
e3 = np.zeros(LoopCount) # storage layer ET rate (mm/hr)
f1 = np.zeros(LoopCount) # infiltration rate of surface water into the soil layer (mm/hr)
F1 = np.zeros(LoopCount) # accumulative infiltraion of surface water into the soil layer (mm)
f2 = np.zeros(LoopCount) # percolation rate of water through the soil layer into the storage layer (mm/hr)
...
delta[0] = 4098*es2[0]/(Tair[0]+237.3)**2     #Slope of saturation vapour pressure curve，kPa/℃
Rn[0] = (1-albedo)*Rsolar[0] #净太阳辐射，W/m2，不考虑长波辐射的损失
if d1_0 == 0:
    e1[0] = 0
else:
  e1[0] = (0.408*delta[0]*0.8*Rn[0]+y[0]*37.5/(Tair[0]+273)*Uwind[0]*(es2[0]-ea2[0]))/(delta[0]+y[0]*(1+0.34*Uwind[0]))

#考虑水分胁迫的基质层蒸散，mm/h
e2[0] = Ks[0]*(0.408*delta[0]*0.8*Rn[0]+y[0]*37.5/(Tair[0]+273)*Uwind[0]*(es2[0]-ea2[0]))/(delta[0]+y[0]*(1+0.34*Uwind[0]))
...
for t in range(1,LoopCount):
    ...
    delta[t] = 4098*es2[t]/(Tair[t]+237.3)**2     #Slope of saturation vapour pressure curve，kPa/℃

    if d1[t-1] > 0:
        e1[t] = (0.408*delta[t]*0.8*Rn[t]+y[t]*37.5/(Tair[t]+273)*Uwind[t]*(es2[t]-ea2[t]))/(delta[t]+y[t]*(1+0.34*Uwind[t]))
    else:
        e1[t] = 0
        
    e2[t] = Ks[t]*(0.408*delta[t]*0.8*Rn[t]+y[t]*37.5/(Tair[t]+273)*Uwind[t]*(es2[t]-ea2[t]))/(delta[t]+y[t]*(1+0.34*Uwind[t]))

    #砾石层水分蒸发速率，mm/hr
    if d3[t-1]>0:
        e3[t] = C1*(1-xita2[t-1]/phi2)*e0*np.exp(17.62*gravelT[t]/(24.3+gravelT[t]))
    else:
        e3[t] = 0
