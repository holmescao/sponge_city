...
def fig_rainfall_runoff(rain,runoff, name_list,width,height, start_dt_str, freq):
    ...
    # 设置x坐标轴标签和刻度、字体
    xticks, xlabels = get_xticklabels(start_dt_str,periods=len(rain), num=3, freq=freq,fmt="%H:%M",tail=True)
...
def fig_rainfall_runoff_v0(rain,runoff, name_list,width,height, start_dt_str,periods, freq):
    F1 = MyFigure(width=width, height=height, dpi=150)
    F1.resize(width,height)
    F1.axes1 = F1.fig.add_subplot(111) 
    ...
    # rain
    ax2 = F1.axes1.twinx()
    ax2.bar(xx_pred, rain, width=1,fc='b')
    ax2.tick_params(axis='y',labelsize=fs)
    ax2.set_ylim((rain.max()*3, 0))
    # ax2.invert_yaxis() # 翻转坐标轴
    ax2.set_ylabel(u"降雨量 (mm)", fontsize=fs-5)
    ax2.tick_params(axis='y', labelcolor="b") # 坐标轴颜色
    
    # runoff
    for c in range(col):
        F1.axes1.plot(xx_pred,runoff[:,c],
                      color=color_list[c],lw=1,linestyle="--",
                      marker=all_marker_list[c],label=name_list[c])
    F1.axes1.legend()
    
    xticks, xlabels = get_xticklabels(start_dt_str,periods, num=3, freq=freq,fmt="%H",tail=False)

    # F1.axes1.legend(fontsize=fs-2)
    F1.axes1.set_title(start_dt_str.replace(" ","T").split("T")[0])
    F1.axes1.set_xlabel(u"时间(min)", fontsize=fs-2)
    F1.axes1.set_ylabel(u"径流量 (mm)", fontsize=fs-5)
...
def fig_rainfall(values, width,height, start_dt_str, freq):
    ...
    xticks, xlabels = get_xticklabels(start_dt_str,periods=len(values), num=3, freq=freq,fmt="%H:%M",tail=True)
...
def fig_runoff(values, name_list, width,height, start_dt_str, freq):
    ...
    all_marker_list = ["o","v","s","p","P","1", "h","D","X"]
    for c in range(col):
        F1.axes1.plot(xx_pred,values[:,c],color=color_list[c],lw=1,linestyle="--",marker=all_marker_list[c],label=name_list[c]+"模拟值")
...
def fig_vs_single_pollution(metric, vs_pollution, name_list,width,height, start_dt_str, freq):
    ...
    xticks, xlabels = get_xticklabels(start_dt_str,periods=length, num=3, freq=freq,fmt="%H:%M",tail=True)
...
def fig_pollution(values, width,height, start_dt_str, freq):
    ...
    xticks, xlabels = get_xticklabels(start_dt_str,periods=length, num=3, freq=freq,fmt="%H:%M",tail=False)
...
def fig_rain_generate(values, width,height):
    ...
    ax.plot(xx,values,color='b',lw=1,linestyle="-", marker='s',markersize=3,label='降雨强度')
